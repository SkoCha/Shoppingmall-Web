		...
		<div class="row">
        	<div class="col-md-12">
                	...
                	<tr>
                		...
                		<td>
                			<form role="form" action="/admin/shop/orderView" method="post" class="deliveryForm">
                			<input type="hidden" name="orderId" value="${orderView.orderId}">
                			<input type="hidden" name="delivery" class="delivery" value="">
                			<label>${orderView.delivery}</label>
                			<button type="submit" class="btn btn-primary pull-right delivery_btn">배송 완료</button>
                			<button type="submit" class="btn btn-primary pull-right delivery_btn">배송 중</button>
                			<button type="submit" class="btn btn-primary pull-right delivery_btn">배송 준비중</button>
                				<script>
                					$(".delivery_btn").click(function(e){
                						e.preventDefault();
                						var conf = confirm("배송 상태를 변경하시겠습니까?");
                						if(conf) {
	                						var deliverObj = $(this).text();
	                						var formObj = $(".deliveryForm");
	                						console.log(deliverObj);
	                						$("input[name='delivery']").val(deliverObj);
	                						formObj.submit();                							
                						}
                					});
                				</script>
